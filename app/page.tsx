...
function NewsItem({ tag, source, content }: NewsItemProps) {
  return (
    <div className="border-b border-slate-800 pb-4 last:border-0 last:pb-0 hover:bg-slate-800/30 transition-colors p-1 rounded">
...
export default function Home() {
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 font-sans selection:bg-blue-500/30">
      {/* 헤더 */}
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-end mb-10 border-b border-slate-800 pb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium italic">실시간 전쟁 및 국제 분쟁 상황판 | <span className="text-blue-500 not-italic">world-war.kr</span></p>
        </div>
        <div className="text-right text-xs text-slate-600 bg-slate-900 px-3 py-1 rounded-full border border-slate-800">
          마지막 업데이트: <span className="text-slate-400">실시간 감시 중</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
          <h2 className="text-lg font-bold mb-6 flex items-center border-l-4 border-blue-600 pl-3">실시간 주요 뉴스</h2>
        ...
        {/* 지도 섹션: 높이를 h-[600px]로 고정하여 지도 가독성을 높였습니다. */}
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl h-[600px] relative ring-1 ring-slate-800">
          <Map
            initialViewState={{
              longitude: 35.0, 
              latitude: 35.0,  
          ...
          >
            ...
            <Marker longitude={30.52} latitude={50.45} anchor="bottom">
              <div className="w-5 h-5 bg-red-600 rounded-full border-2 border-white animate-pulse shadow-[0_0_15px_rgba(220,38,38,1)]"></div>
            </Marker>

            {/* 마커: 중동 */}
            <Marker longitude={35.21} latitude={31.76} anchor="bottom">
              <div className="w-5 h-5 bg-orange-500 rounded-full border-2 border-white animate-pulse shadow-[0_0_15px_rgba(249,115,22,1)]"></div>
            </Marker>
          </Map>
        </div>
      </div>
