  ...
  const loadConflictData = async (type: 'MIDDLE_EAST' | 'RUSSIA_UKRAINE') => {
    ...
    if (type === 'MIDDLE_EAST') {
      setStats({
        ...
        days: getDiffDays("2026-02-28").toString(), // 23일차
      ...
      // 💡 암살(Assassination) 및 호르무즈(Hormuz) 키워드 강화
      query = 'Iran Israel USA "Strait of Hormuz" Assassination conflict';
    } else if (type === 'RUSSIA_UKRAINE') {
      setStats({
        name: "러시아-우크라이나 전쟁",
        days: getDiffDays("2022-02-24").toString(),
        deaths: { val: "500,000+", src: "ACLED / UN HR Monitoring" },
        damage: { val: "$486B", src: "IMF / World Bank" },
        oil: { val: "102.15", src: "Qatar Marine (QP)" }
      });
      setActiveTab('Russia-Ukraine');
      query = 'Russia Ukraine war military';
    }

    try {
      // 💡 최신순(publishedAt) 정렬과 검색어 조합으로 기사가 안 뜨는 문제 완전 해결
      ...
      if (data.articles && data.articles.length > 0) {
        ...
        // API 결과가 없을 경우를 대비한 백업 쿼리 시도
        const backupRes = await fetch(`https://gnews.io/api/v4/search?q=${encodeURIComponent(type === 'MIDDLE_EAST' ? "Iran war" : "Ukraine war")}&lang=en&max=5&apikey=${GNEWS_API_KEY}`);
        const backupData = await backupRes.json();
        setNews(backupData.articles || []);
      }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12 font-sans selection:bg-red-500/30">
      <div className="max-w-7xl mx-auto flex justify-between items-end mb-10 border-b border-slate-800 pb-6">
        ...
        {stats && (
          <button onClick={() => { setStats(null); setNews([]); setActiveTab('Idle'); }} className="text-[10px] font-bold bg-slate-900 px-4 py-2 rounded border border-slate-700 hover:bg-red-900 transition-all uppercase">CLOSE INTEL</button>
        )}
      </div>
      ...
      <div className="max-w-7xl mx-auto min-h-[180px] mb-10 transition-all">
        {!stats ? (
          <div className="w-full bg-slate-900/30 border-2 border-dashed border-slate-800 rounded-3xl p-8 flex flex-col items-center justify-center">
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mb-6 animate-pulse">Select Active Conflict to Receive News</p>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
        ...
        <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-3xl p-6 h-[550px] flex flex-col shadow-2xl overflow-hidden relative">
          {!stats && <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-[1px] z-10 flex items-center justify-center text-[10px] text-slate-600 font-bold uppercase tracking-widest">Feed Locked</div>}
          ...
          <div className="space-y-4 overflow-y-auto flex-1 pr-2 scrollbar-hide">
            {loading ? (
              <p className="text-[10px] text-red-600 animate-pulse uppercase font-black">Decrypting Data...</p>
            ) : (
              news.map((item, idx) => (
                <div key={idx} className="border-b border-slate-800/50 pb-4 last:border-0 cursor-pointer hover:bg-slate-800/40 p-3 rounded transition-all" onClick={() => window.open(item.url)}>
          ...
          >
            ...
            <Marker longitude={56.3} latitude={26.6} onClick={() => loadConflictData('MIDDLE_EAST')}>
              <div className="cursor-pointer group relative">
                ...
                <div className="w-8 h-8 bg-red-600 rounded-full border-2 border-white shadow-[0_0_20px_red]"></div>
              </div>
            </Marker>
            <Marker longitude={37.6} latitude={48.3} onClick={() => loadConflictData('RUSSIA_UKRAINE')}>
              <div className="cursor-pointer group relative">
                ...
                <div className="w-8 h-8 bg-blue-600 rounded-full border-2 border-white shadow-[0_0_20px_blue]"></div>
              </div>
            </Marker>
          </Map>
        </div>
...
function ConflictButton({ name, onClick }: any) {
  return (
    <button onClick={onClick} className="px-6 py-3 bg-red-600/10 border border-red-600/30 rounded-xl text-[11px] font-bold text-red-500 hover:bg-red-600 hover:text-white transition-all uppercase shadow-lg shadow-red-900/10">
      {name}
    </button>
  );
}
